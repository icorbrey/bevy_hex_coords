...
fn setup(
    ...
    let verts: std::collections::HashSet<HexVert> =
        coords.iter().flat_map(|c| c.vertices()).collect();

    let edges: std::collections::HashSet<HexEdge> = coords.iter().flat_map(|c| c.edges()).collect();
